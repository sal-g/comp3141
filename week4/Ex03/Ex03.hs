...
mysteryPred i (Branch t l r) | i > t = mysteryPred i r
                             | i < t = mysteryPred i l
                             -- if i == t
                             | otherwise = True
  -- error "'mysteryPred' unimplemented"
...
-- concat all left then root then all right
mysterious (Branch t l r) = mysterious l ++ [t] ++ mysterious r 
{-
 - _ means can be anything as long as its the right type
 - this case can't be first case since it will 
 - take valid arg and return an empty List
 -}
mysterious _ = [] 
  -- error "'mysterious' unimplemented"
...
astonishing [] = Leaf -- empty list
astonishing xs = Branch root (astonishing left) (astonishing right) 
  where
   root = xs !! div (length xs) 2
   (l,r) = splitListTo2 xs
   left = init l -- remove the root
   right = r
  -- error "'astonishing' unimplemented"

splitListTo2 :: [Integer] -> ([Integer],[Integer])
splitListTo2 xs = splitAt (div (length xs) 2 + 1) xs
